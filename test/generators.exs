defmodule Elixoids.Test.Generators do
  ...
  # 1.05 .. ~2.0
  def double_radius,
    do: :triq_dom.bind(smaller_radius(), fn f -> 1.1 + f end)
  ...
  defp points_circle(radii, {delta_r1, delta_r2}) do
    [gen_point(), radii, delta_r1, gen_theta(), delta_r2, gen_theta()]
    |> :triq_dom.bind(fn [p, r, dr1, t1, dr2, t2] ->
      dx1 = Float.floor(:math.cos(t1) * (r * dr1), 1)
      dy1 = Float.floor(:math.sin(t1) * (r * dr1), 1)
      dx2 = Float.floor(:math.cos(t2) * (r * dr2), 1)
      dy2 = Float.floor(:math.sin(t2) * (r * dr2), 1)
      p1 = Point.translate(p, dx1, dy1)
      p2 = Point.translate(p, dx2, dy2)
      {%{pos: p, radius: r}, {p1, p2}}
    end)
  end
  ...
  defp points_impale_circle(radii) do
    ...
    |> :triq_dom.bind(fn [p, r, fr, t0, t, dr1, dr2] ->
      ox = Float.floor(:math.cos(t0) * (r * fr), 1)
      oy = Float.floor(:math.sin(t0) * (r * fr), 1)
      ...
      o = Point.translate(p, ox, oy)
      p1 = Point.translate(o, dx1, dy1)
      p2 = Point.translate(o, dx2, dy2)
      {%{pos: p, radius: r}, {p1, p2}}
    end)
  end

  defp points_outside_centre_circle(radii) do
    [gen_point(), gen_theta(), radii, double_radius(), larger_radius()]
    |> :triq_dom.bind(fn [p, t, r, dr1, dr2] ->
      dx1 = Float.floor(:math.cos(t - 0.35) * (r * dr1 * 1.2), 1)
      dy1 = Float.floor(:math.sin(t - 0.35) * (r * dr1 * 1.2), 1)
      dx2 = Float.floor(:math.cos(t + 0.35) * (r * dr2 * 1.2), 1)
      dy2 = Float.floor(:math.sin(t + 0.35) * (r * dr2 * 1.2), 1)
      p1 = Point.translate(p, dx1, dy1)
      p2 = Point.translate(p, dx2, dy2)
      {%{pos: p, radius: r}, {p1, p2}}
    end)
  ...
  def gen_bullet_point,
    do: :triq_dom.bind(gen_point(), fn p -> %BulletLoc{pos: [p]} end)

  defp gen_bullet_points,
    do:
      :triq_dom.bind([gen_point(), gen_theta()], fn [p, theta] ->
        %BulletLoc{pos: [p, Point.move(p, theta, 50.0)]}
      end)

  def gen_bullet,
    do: oneof([gen_bullet_point(), gen_bullet_points()])
  ...
  def line_outside_asteroid,
    do: points_outside_centre_circle(asteroid_radius())
end
